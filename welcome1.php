...
<button class="but" onclick="func1()"><b>MY SHIPS</b></button>
...
<br ><br >
  ...
  <br><br>
    ...
    $res=$con->query($st);
      ...
      if($con->query($hu)){
        $alkas="SELECT Email FROM Authority ORDER BY RAND() LIMIT 2";
        $rek=$con->query($alkas);
        if($rek->num_rows>0){
          while($rowq=$rek->fetch_assoc()){
            $to=$rowq['Email'];
            $subject="Docking request";
            // $from=
            $txt="Greetings, I'd like to reserve a dock for the ".$s.". I hope you will take my request into consideration.";
    ...
    $res=$con->query($st);
      ...
      if($con->query($hhu)){
        $alka="SELECT Email FROM Authority ORDER BY RAND() LIMIT 2";
        $rekd=$con->query($alka);
        if($rekd->num_rows>0){
          while($rowq=$rekd->fetch_assoc()){
            $to=$rowq['Email'];
            $subject="Departure request";
            // $from=
            $txt="Greetings, I'd like to vacate dock no. ".$s.". I hope you will take my request into consideration and give me permision to leave at the given time.";
            // $email="rishithreddyaduma@gmail.com";
            $header = "From:abc@somedomain.com \r\n";
                $header .= "Cc:afgh@somedomain.com \r\n";
                $header .= "MIME-Version: 1.0\r\n";
